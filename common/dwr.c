...
static void dwr_speed_hacks(dw_rom *rom)
{
	...
	IF (MEDIUM_TEXT(rom)) {
		printf("Enabling medium text...\n");
		
        /* speed up the text, 2 characters / frame */
        vpatch(rom, 0x7a44, 2, 0x0a, 0xbf); // JSR BF0A instead of FF74
        // Refresh every 2 characters instead of 1.
        vpatch(rom, 0x7f1a, 2, 0xa5, 0xd2,  // LDA $0002
			0x29, 0x01, // AND #01
			0xd0, 0x03, // BNE to JSR
			0x20, 0x74, 0xff, // JSR FF74 (repaint the canvas)
			0x60); // JSR
	}
	
	if (DOUBLE_WALKING(rom)) {
		printf("Doubling walking speed...\n");
		
		/* double walking speed */
        // Redirect to other bytes
		vpatch(rom, 0x32a5, 2, 0x80, 0xa5); // NEXT 2 LINES:  JSR A580
		vpatch(rom, 0x339f, 2, 0x80, 0xa5);
		vpatch(rom, 0x3423, 2, 0x90, 0xa5); // NEXT 7 LINES:  JSR A590
		vpatch(rom, 0x3439, 2, 0x90, 0xa5);
		vpatch(rom, 0x3466, 2, 0x90, 0xa5);
		vpatch(rom, 0x3490, 2, 0x90, 0xa5);
		vpatch(rom, 0x3578, 2, 0x90, 0xa5);
		vpatch(rom, 0x35a5, 2, 0x90, 0xa5);
		vpatch(rom, 0x35cf, 2, 0x90, 0xa5);
		// Change sprite walking refresh to 8 frames instead of 16.  
		// Otherwise, the sprite will be stuck during double walking speed.
		vpatch(rom, 0x36f4, 1, 0x07); 
        // Refresh the canvas every 2 "ministeps" instead of 1.
		vpatch(rom, 0x2590, 16, 
			0xA5, 0x45, // LDA $0045
			0xC9, 0x01, // CMP #01
			0xD0, 0x06, // BNE to JSR FF74
			0xA5, 0x05, // LDA $0005 - Horizontal position
			0x29, 0x01, // AND #01
			0xD0, 0x03, // BNE to JSR
			0x20, 0x74, 0xFF, // JSR FF74 (repaint the canvas)
			0x60); // JSR
		vpatch(rom, 0x25a0, 16, 
			0xA5, 0x45, 
			0xC9, 0x01, 
			0xD0, 0x06, // Same as previous patch,
			0xA5, 0x07, // except LDA $0007 - vertical position
			0x29, 0x01, 
			0xD0, 0x03, 
			0x20, 0x74, 0xFF, 
			0x60);
